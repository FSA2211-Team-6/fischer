...
export default function SingleComment({ comment, handleDeleteComment }: any) {
  ...
  const handleFormSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    editComment(content, comment.id);
    setIsEditing(false);
  };

  const editComment = async (content: number, commentId: number) => {
    const commentData = {
      content: content,
      commentId: commentId,
    };
    try {
      const putResponse = await fetch(
        `https://fischer.onrender.com/api/comments/${commentId}`,
        {
          method: "PUT",
          body: JSON.stringify(commentData),
        }
          ...
          <div className="flex flex-row">
            ...
            <div className="flex-col mt-1 w-full">
              ...
              <div className="flex gap-2 items-start">
                ...
                {userId === comment.fischerId ? (
                  <button
                    onClick={(e: any) => handleDeleteComment(comment.id)}
